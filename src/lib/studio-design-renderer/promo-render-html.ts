/**
 * Deterministic HTML renderer for one promo asset (canvas-sized).
 */

import { readFileSync } from "fs";
import path from "path";

import type { PromoAssetSpec, PromoCampaignSetSpec } from "./promo-types";
...
export function declaredTextFromPromoAsset(asset: PromoAssetSpec): string {
  return asset.layers
...
export function renderPromoAssetHtml(
  repoRoot: string,
  setSpec: PromoCampaignSetSpec,
...
<head>
<meta charset="utf-8" />
<title>Promo ${esc(asset.assetId)}</title>
<style>
  html, body {
    margin: 0;
    padding: 0;
...
<body>
  <div class="canvas" data-asset-id="${esc(asset.assetId)}" data-purpose="${esc(asset.authorizedPurpose)}" data-plate="${esc(asset.plateId)}">
    ${parts.join("\n    ")}
  </div>
</body>
</html>
