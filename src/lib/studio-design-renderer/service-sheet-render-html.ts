/**
 * Deterministic HTML/CSS renderer for ServiceSheetDesignSpec.
 * Same absolute-layer capture contract as menu (reuse captureMenuExports).
 */

import { readFileSync } from "fs";
import path from "path";

import type { ServiceSheetDesignSpec } from "./service-sheet-types";
...
export function renderServiceSheetHtml(
  repoRoot: string,
  ...
  for (const layer of spec.layers) {
    ...
    const tracking =
      layer.letterSpacingPx != null
        ...
        : "";
    const height = layer.height != null ? `height:${layer.height}px;` : "";
    const minFont = layer.minFontPx != null ? String(layer.minFontPx) : "";
    const svcAttr = layer.serviceId
      ? ` data-service-id="${esc(layer.serviceId)}"`
      : "";
    parts.push(
      `<div class="layer text" data-id="${esc(layer.id)}" data-role="${esc(layer.role)}" data-min-font="${esc(minFont)}"${svcAttr} style="left:${layer.x}px;top:${layer.y}px;width:${layer.width}px;${height}font-size:${layer.fontSizePx}px;font-weight:${layer.fontWeight};line-height:${layer.lineHeight};color:${esc(layer.color)};text-align:${layer.align};${tracking}">${esc(layer.content)}</div>`,
    );
  }
...
<head>
<meta charset="utf-8" />
<title>Service Sheet ${esc(spec.skuId)}</title>
<style>
  html, body {
    margin: 0;
    padding: 0;
...
export function declaredTextFromServiceSheetSpec(
  spec: ServiceSheetDesignSpec,
): string {
  return spec.layers
