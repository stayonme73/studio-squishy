/**
 * Deterministic HTML/CSS renderer for FlyerDesignSpec (v2-rtu-flyer only).
 */

import { readFileSync } from "fs";
import path from "path";

import type { FlyerDesignSpec } from "./types";
...
export function renderFlyerHtml(
  repoRoot: string,
...
<head>
<meta charset="utf-8" />
<title>Flyer ${esc(spec.skuId)}</title>
<style>
  html, body {
    margin: 0;
    padding: 0;
...
<body>
<main class="canvas" data-sku="${esc(spec.skuId)}" data-spec="${esc(spec.specVersion)}">
${parts.join("\n")}
</main>
</body>
</html>
...
export function declaredTextFromSpec(spec: FlyerDesignSpec): string {
  return spec.layers
