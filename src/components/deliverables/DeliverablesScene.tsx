...
/** Repository convention: client preview gates use inlined NODE_ENV. */
...
function JobDeliveryGrid({
  jobs,
  emptyFilesCopy,
}: {
  jobs: readonly ClientJobDeliveryView[];
  emptyFilesCopy: string;
}) {
  return (
        ...
        <article key={job.jobId} className="fd-block fd-block--scope">
          ...
          <div className="fd-block__body">
            ...
            {job.files.length > 0 ? (
              ...
              <p className="fd-item__text">{emptyFilesCopy}</p>
            )}
          </div>
        </article>
      ))}
...
function PreviewPackageGrid({ pkg }: { pkg: CampaignDeliverablesPackage }) {
  return (
    <div className="fd-deliverables__grid" data-preview-package="true">
...
export default function DeliverablesScene({ embedded = false }: DeliverablesSceneProps) {
  ...
  const previewDevelopmentOnly = isDevelopmentRuntime() && previewQueryRequested;

  const { campaign, ready, accessState, refresh } = useCurrentCampaign();
  const { state: deliveryState, refresh: refreshDelivery } = useFinalDelivery(
    previewDevelopmentOnly ? undefined : campaign?.campaignId,
  );

  const finalDelivery = deliveryState.status === "ready" ? deliveryState.delivery : null;

  const view = useMemo(
    () =>
      resolveDeliverablesView(campaign, {
        previewDevelopmentOnly,
        finalDelivery: previewDevelopmentOnly ? null : finalDelivery,
      }),
    [campaign, previewDevelopmentOnly, finalDelivery],
  );
  ...
  const partialCopy =
    view.partialVariant === "some-files-missing"
      ? HONEST_DELIVERY_COPY.partialSomeMissing
      : HONEST_DELIVERY_COPY.partialInProgress;

  const heroBadge =
    view.state === "files-ready"
      ? hero.badge
  ...
  const heroTitle =
    view.state === "files-ready"
      ...
      : view.state === "preview-development-only"
        ...
        : view.state === "delivered-no-files"
          ? HONEST_DELIVERY_COPY.deliveredNoFiles.title
          : partialCopy.title;
  ...
  const heroLead =
    view.state === "files-ready"
      ? hero.lead
      : view.state === "preview-development-only"
        ? "Generated preview content for local development only. This is not released customer work."
        : view.state === "delivered-no-files"
          ? HONEST_DELIVERY_COPY.deliveredNoFiles.body
          : partialCopy.lead;

  const emptyFilesCopy =
    view.state === "delivered-no-files" || view.partialVariant === "some-files-missing"
      ? HONEST_DELIVERY_COPY.deliveredNoFiles.perJob
      : jobDelivery.noFiles;

  const showDownloadSection =
    view.state === "files-ready" ||
    view.state === "partial-files-ready" ||
