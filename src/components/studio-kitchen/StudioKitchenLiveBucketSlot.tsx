import StudioKitchenLiveFolderCard from "@/components/studio-kitchen/StudioKitchenLiveFolderCard";
...
import type { KitchenLiveBucketSlotView } from "@/lib/studio-kitchen";

type Props = {
  slot: KitchenLiveBucketSlotView;
  onOpenFolder: (campaignId: string) => void;
};

export default function StudioKitchenLiveBucketSlot({ slot, onOpenFolder }: Props) {
        ...
        {slot.folders.length === 0 ? (
          ...
          <ol className="sk-bucket-slot__folders">
            {slot.folders.map((folder) => (
              <li key={folder.campaignId}>
                <StudioKitchenLiveFolderCard
                  folder={folder}
          ...
          {slot.tray.folders.length > 0 ? (
            <ul className="sk-tray__folders">
              {slot.tray.folders.map((folder) => (
                <li key={folder.campaignId}>
                  <StudioKitchenLiveFolderCard folder={folder} onOpen={onOpenFolder} />
                </li>
              ))}
            </ul>
          ) : (
            <p className="sk-tray__empty">Empty</p>
          )}
        </div>
      ) : null}
    </div>
