...
| L6 | No scheduled heartbeat | **In progress / blocked for external wake.** Machine sweep runs in-process while Node is awake. Live two-process Supabase proof passed. No external scheduler. External wake is also blocked by L14. Hydrate-only never-ran is eventual (L15), not a scheduler-death alarm. Do not connect a scheduler until Tagia authorizes the wake-runtime build and a later observer cert. |
...
| L14 | Private certification host vs noninteractive Machine wake | **Open. Architecture conditionally approved 2026-08-24. Not implemented.** O2: private host 401 HTML; Studio auth never ran; not a bad secret. Keep the Studio site private. Stop sweep tests on that host. Wake runtime must execute existing `machine.sweep()` against the same Supabase schema v2 and **must not proxy** to the private host. Provider request cap required **before** handler auth. Authenticated cap is **18 unique successful wakes / rolling hour**; idempotent replays do not consume it. Durable wake idempotency is `supervision_wake_idempotency` + `supervision_claim_wake_idempotency` / `supervision_complete_wake_idempotency` (same project; not process memory). Store-down after auth: sanitized 503, no claimed durable failed-to-run write. No scheduler yet. Decision note: `MACHINE-ONLY-WAKE-INGRESS-DECISION-NOTE.md`. Implementation contract: `MACHINE-ONLY-WAKE-RUNTIME-IMPLEMENTATION-CONTRACT.md`. |
