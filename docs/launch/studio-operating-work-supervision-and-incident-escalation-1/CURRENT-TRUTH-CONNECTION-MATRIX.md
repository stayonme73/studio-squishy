...
| Machine records | Campaign, jobs, exceptions, plus in-process Machine incident and lease store | Local store + process memory | Yes for production records; supervision core is in-process only | Production events; fictional supervision fixtures and live proof leases | Authenticated register/heartbeat/sweep | In-process sweep while Node is awake; authenticated sweep wake; no external scheduler | Lease stall, dead service, mismatch, overdue next check | Yes on those records; fixtures isolated from live proof runtime | Desk folders + additive Incident Command | **Partial — in-process only, not persisted, no external scheduler** |
