...
**State:** OPEN / IN PROGRESS — **Foundation Pass 1 accepted. Runtime Pass 2 accepted. Durable Pass 3 accepted for local/controlled proof only. Pass 3B launch-runtime durability correction in progress. Not closed. Not launch-certified.**
...
- Provider class: memory = unit tests only; `studio-data-json` = local development and controlled certification only; launch production requires `supabase-postgres`. Production fails closed otherwise.  
