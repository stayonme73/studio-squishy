/**
 * Slice 3c-a — QA PATCH API (no QA UI)
 *
 * Prerequisites:
 *   - Dev server on localhost:3000
 *   - SESSION_SECRET in .env.local
 *
 * Usage: node scripts/verify-slice3c-a.mjs
 */
...
const COPY_CHECKS = ["copy_accuracy", "brand_voice", "grammar"];
const QA_CHECKS = ["production_complete", "deliverable_specs", "client_requirements"];
const DELIVERY_CHECKS = ["package_complete", "fingerprint_match", "no_internal_leaks"];
...
async function main() {
  ...
  await writeFile(
    path.join(process.cwd(), "tmp", "verify-slice3c-a-report.json"),
