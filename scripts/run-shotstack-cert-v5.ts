/**
 * KITCHEN-PRODUCTION-CERT-VIDEO-1 — Production V5 (SKU narration + A/V sync).
...
async function main() {
  ...
  const summary: Record<string, unknown> = {
    ...
    tagiaEditing: false,
    correctionFocus: "sku_appropriate_narration_then_av_sync",
  ...
  if (!v1Key) {
    ...
    writeFileSync(
      path.join(evidenceDir, "v5-run-summary.json"),
  ...
  summary.preserved = {
    v1: assertV1Preserved(repoRoot, packet),
    v2: {
      ok: Boolean(
    ...
    v4: {
      ok: Boolean(
        packet.preserveV4RelativePath &&
          existsSync(path.join(repoRoot, packet.preserveV4RelativePath)),
      ),
      path: packet.preserveV4RelativePath,
    },
  };
  ...
  summary.v5 = {
    ...
    voiceArtifact: packet.voiceArtifact,
    scriptVersionId: packet.scriptVersionId,
  };
  summary.sceneToScriptMap = packet.sceneToScriptMap;
  summary.machineQa = qa;
  summary.verdict = qa.ok
    ? "V5 CANDIDATE READY FOR OWNER VISUAL REVIEW — NOT CERTIFIED"
    : "V5 MACHINE QA FAILED — NOT CERTIFIED";
  summary.readiness =
    "INTEGRATED / QA READY / NOT CUSTOMER READY / NOT CERTIFIED";
  ...
  writeFileSync(
    path.join(evidenceDir, "v5-machine-qa.json"),
