/**
 * Scout kiosk hotspot visibility across viewports (contain framing math).
 */
const native = { width: 1024, height: 683 };
const kiosk = { x: 768, y: 318, width: 210, height: 340 };
const framingDefault = { x: 0.5, y: 0.5, fit: "contain" };
...
function letterbox(viewport, framing) {
  const scale = Math.min(viewport.width / native.width, viewport.height / native.height);
  const rw = native.width * scale;
  const rh = native.height * scale;
  const barX = viewport.width - rw;
  const barY = viewport.height - rh;
  return { barX, barY, renderedW: rw, renderedH: rh };
}
...
for (const vp of viewports) {
  ...
  console.log(
    `${vp.name}: ${pass ? "PASS" : "FAIL"} fit=${framing.fit} letterbox=${lb.barX.toFixed(0)}px sides ${lb.barY.toFixed(0)}px top/bottom center=(${centerX.toFixed(1)}%,${centerY.toFixed(1)}%) box=${JSON.stringify(pct)}`,
  );
}
